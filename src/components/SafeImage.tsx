...
export const SafeImage = ({ src, alt, className, fallbackText, ...props }: SafeImageProps) => {
  const [error, setError] = useState(false);

  if (error || !src) {
    return (
      <div className={`flex flex-col items-center justify-center bg-brand-cream/50 text-brand-dark/20 gap-3 p-8 ${className}`}>
